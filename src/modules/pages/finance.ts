...
function renderKpiCards(report: ReturnType<typeof financialReportRoute>): string {
  const k = report.kpis;
  return `
  <section class="cards">
...
function renderReportTables(report: ReturnType<typeof financialReportRoute>): string {
  return `
    <div class="grid-two">
      <section class="panel">
...
function renderEntriesTables() {
  ...
  const table = (rows: typeof entries, kind: 'PAGAR' | 'RECEBER') => rows.length === 0
...
export function renderFinance(): string {
...
export function bindFinance(onDone: () => void): void {
  document.getElementById('financePeriodForm')?.addEventListener('submit', (e) => {
    ...
    if (kpiEl) kpiEl.innerHTML = renderKpiCards(report);
    if (tableEl) tableEl.innerHTML = renderReportTables(report);
  });
  ...
  document.getElementById('categoryForm')?.addEventListener('submit', (e) => {
    ...
    const type = String(fd.get('type') || 'AMBOS') as 'RECEITA' | 'DESPESA' | 'AMBOS';

    if (!validateRequired([['Nome da categoria', name]])) return;
    const result = createFinancialCategoryRoute({ name, type });
    toast(result.message, result.ok ? 'success' : 'error');
    onDone();
  });

  function submitEntry(kind: 'payable' | 'receivable', form: HTMLFormElement) {
    ...
    if (!validateRequired([['Categoria', categoryId], ['Descrição', description], ['Vencimento', dueDate]])) return;

    const category = listFinancialCategoriesRoute().find((c) => c.id === categoryId);
    if (!category) return toast('Categoria inválida.', 'error');
  ...
  document.getElementById('payableForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    submitEntry('payable', e.target as HTMLFormElement);
  });

  document.getElementById('receivableForm')?.addEventListener('submit', (e) => {
    e.preventDefault();
    submitEntry('receivable', e.target as HTMLFormElement);
  });
  ...
  document.getElementById('financeEntries')?.addEventListener('click', (e) => {
    ...
    if (!methodRaw) return;
    const method = methodRaw.toUpperCase() as PaymentMethod;
    if (!PAYMENT_OPTIONS.includes(method)) return toast('Forma de pagamento inválida.', 'error');
