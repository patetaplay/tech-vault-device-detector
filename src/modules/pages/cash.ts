...
export function renderCash(): string {
...
export function bindCash(onDone: () => void): void {
  const typeSelect = document.querySelector<HTMLSelectElement>('#cashTransactionForm select[name="type"]');
  const catSelect = document.getElementById('cashCategory') as HTMLSelectElement | null;
  ...
  document.getElementById('cashTransactionForm')?.addEventListener('submit', (e) => {
    ...
    if (!validateRequired([['Descrição', description], ['Categoria', category]])) return;
