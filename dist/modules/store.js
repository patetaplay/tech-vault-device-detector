...
export function getState() {
    ...
    return {
        ...
        cashTransactions: parsed.cashTransactions || (parsed.cashEntries ?? []),
        sales: (parsed.sales || []).map((s) => ({
            ...s,
            items: (s.items || []).map((i) => ({ unitCost: i.unitCost ?? 0, ...i }))
        })),
...
function registerCashTransactionInternal(state, input) {
    const session = getOpenCashSession(state);
    if (!session)
        return { ok: false, message: 'Abra o caixa antes de registrar movimentações.' };
    if (input.type === 'SAIDA' && !input.category.trim())
        return { ok: false, message: 'Categoria é obrigatória para saída financeira.' };
...
function addFinancialEntryInternal(state, entry) {
    const created = { id: crypto.randomUUID(), createdAt: nowIso(), ...entry };
...
export function registerStockAdjustment(input) {
    const state = getState();
...
export function updateOrder(orderId, patch) {
    ...
    if (order.status === 'ENTREGUE' || order.status === 'CANCELADA')
        return { ok: false, message: 'Não é possível editar uma OS entregue/cancelada.' };
...
export function addOrderTechnicalNote(orderId, note) {
    const state = getState();
    const order = state.orders.find((item) => item.id === orderId);
    if (!order)
        return { ok: false, message: 'OS não encontrada.' };
...
export function changeOrderStatus(orderId, status) {
    ...
    if (!order)
        return { ok: false, message: 'OS não encontrada.' };
    if (order.status === 'ENTREGUE' || order.status === 'CANCELADA')
        return { ok: false, message: 'OS já finalizada.' };
    order.status = status;
    order.updatedAt = nowIso();
...
export function openCashSession(openingAmount, openedBy = 'Sistema') {
    const state = getState();
    if (getOpenCashSession(state))
        return { ok: false, message: 'Já existe um caixa aberto.' };
    if (openingAmount < 0)
        return { ok: false, message: 'Valor de abertura inválido.' };
...
export function closeCashSession(closingAmount) {
    ...
    const session = getOpenCashSession(state);
    if (!session)
        return { ok: false, message: 'Não há caixa aberto.' };
...
export function addCashEntry(input) {
    const state = getState();
    if (input.amount <= 0)
...
export function createCounterSale(input) {
    const state = getState();
    const session = getOpenCashSession(state);
    if (!session)
        return { ok: false, message: 'Abra o caixa para vender no balcão.' };
    const product = state.products.find((item) => item.id === input.productId);
    if (!product)
        return { ok: false, message: 'Produto não encontrado.' };
    if (input.quantity <= 0)
...
export function deliverOrder(orderId) {
    ...
    if (order.status === 'ENTREGUE')
        return { ok: false, message: 'OS já entregue.' };
    if (order.status === 'CANCELADA')
        return { ok: false, message: 'OS cancelada.' };
...
export function getFinancialReport(from, to) {
    const state = getState();
    ...
    const fluxoByDay = new Map();
    const faturamentoMensalMap = new Map();
    const despesasMensaisMap = new Map();
    const lucroPorOS = [];
    const lucroPorVenda = [];
    for (const order of state.orders) {
        if (!order.deliveredAt || !inRange(order.deliveredAt))
            continue;
        quantidadeOS += 1;
