...
const ORDER_STATUS = ['ABERTA', 'EM_ANALISE', 'AGUARDANDO_PECA', 'EM_MANUTENCAO', 'PRONTA', 'ENTREGUE', 'CANCELADA'];
...
function applyFilters() {
    ...
    if (!body)
        return;
    const query = String(document.getElementById('orderSearch')?.value || '');
...
function fillEditForm(order) {
    ...
    form.elements.namedItem('customerId').value = order.customerId;
    form.elements.namedItem('technician').value = order.technician;
    form.elements.namedItem('model').value = order.model;
    form.elements.namedItem('imei').value = order.imei;
    form.elements.namedItem('defect').value = order.defect;
    form.elements.namedItem('passwordPattern').value = order.passwordPattern;
    form.elements.namedItem('estimatedDelivery').value = order.estimatedDelivery;
    form.elements.namedItem('entryState').value = order.entryState;
    form.elements.namedItem('paymentOnDelivery').value = order.paymentOnDelivery ? 'SIM' : 'NAO';
    form.elements.namedItem('paymentMethod').value = order.paymentMethod;
    const selectedServiceIds = order.services.map((item) => item.serviceId);
    form.querySelectorAll('input[name="serviceIds"]').forEach((input) => {
        input.checked = selectedServiceIds.includes(input.value);
    });
    if (order.products[0]) {
        form.elements.namedItem('productId').value = order.products[0].productId;
        form.elements.namedItem('quantity').value = String(order.products[0].quantity);
    }
...
function handleTableActions(onDone) {
    document.getElementById('ordersTableBody')?.addEventListener('click', (event) => {
        ...
        if (action === 'status') {
            ...
            if (!ORDER_STATUS.includes(next))
                return toast('Status inválido.', 'error');
            const result = changeOrderStatus(order.id, next);
        ...
        if (action === 'print') {
            ...
            if (!popup)
                return toast('Não foi possível abrir a impressão.', 'error');
...
export function bindOrders(onDone) {
    document.getElementById('osForm')?.addEventListener('submit', (e) => {
        ...
        if (!customer)
            return toast('Selecione um cliente válido.', 'error');
        if (!isValidImei(imei))
            return toast('IMEI inválido. Informe 15 dígitos numéricos.', 'error');
        if (!validateRequired([
            ['Técnico', technician],
            ['Modelo', model],
            ['Defeito', defect],
        ...
        if (productId && quantity <= 0)
            return toast('Quantidade da peça inválida.', 'error');
